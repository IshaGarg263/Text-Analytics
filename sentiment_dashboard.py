...
with st.sidebar:
    ...
    page = st.radio(
        ...
        [
            ...
            "📉 ARIMA/SARIMA on Stock Data",
            "⚔️ Head-to-Head: All Models",
            "📋 Summary & Takeaways"
        ],
...
# ═══════════════════════════════════════════════════════════════
    ...
    fig_scatter.add_trace(go.Scatter(
        x=merged['mean_sentiment'], y=merged['Return'], mode='markers',
        marker=dict(color='#0e7c6b', opacity=0.5, size=5), name='Daily Data'
    ))
    # Manual trendline using numpy polyfit
    mask = merged['mean_sentiment'].notna() & merged['Return'].notna()
    slope, intercept = np.polyfit(merged.loc[mask, 'mean_sentiment'], merged.loc[mask, 'Return'], 1)
    x_range = np.linspace(merged['mean_sentiment'].min(), merged['mean_sentiment'].max(), 100)
    fig_scatter.add_trace(go.Scatter(
        x=x_range, y=slope * x_range + intercept, mode='lines',
        line=dict(color='#e74c3c', width=2.5, dash='dash'), name=f'Trend (slope={slope:.4f})'
    ))
    fig_scatter.update_layout(
        title='Daily Mean Sentiment vs Daily Returns',
        xaxis_title='Mean Sentiment Score', yaxis_title='Daily Return',
        height=450, template='plotly_white', font=dict(family='Inter'), title_font_size=15
    )
...
# ═══════════════════════════════════════════════════════════════
# PAGE: ARIMA/SARIMA ON STOCK DATA
# ═══════════════════════════════════════════════════════════════
elif page == "📉 ARIMA/SARIMA on Stock Data":
    st.markdown("""
    <div class="main-header">
        <h1>📉 ARIMA / SARIMA on Stock Data</h1>
        <p>Applying time series models to stock returns — do they work as well as they did on airline passenger data?</p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("""
    <div class="concept-card">
        <h4>Why try ARIMA/SARIMA here?</h4>
        <p>
        ARIMA and SARIMA are powerful for data with trend and seasonality (like airline passengers). But stock returns
        behave very differently — they are noisy, volatile, and often close to a <strong>random walk</strong>.
        This section lets you test whether time series models can extract any predictable pattern from stock return data.
        </p>
    </div>
    """, unsafe_allow_html=True)

    # --- Prepare Data ---
    stock_returns = stock_df[['Date', 'Close', 'Return']].dropna().reset_index(drop=True)
    returns_series = stock_returns['Return'].values

    # --- Stationarity Check ---
    st.markdown('<div class="section-header"><h2>Step 1: Stationarity Check</h2></div>', unsafe_allow_html=True)

    col1, col2 = st.columns(2)
    with col1:
        # ADF on prices
        adf_price = adfuller(stock_df['Close'].dropna(), autolag='AIC')
        badge_p = "result-pass" if adf_price[1] < 0.05 else "result-fail"
        label_p = "Stationary ✅" if adf_price[1] < 0.05 else "Non-Stationary ❌"
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{adf_price[1]:.4f}</div>
            <div class="metric-label">ADF p-value — Stock Prices — <span class="{badge_p}">{label_p}</span></div>
        </div>
        """, unsafe_allow_html=True)
    with col2:
        # ADF on returns
        adf_ret = adfuller(stock_returns['Return'].dropna(), autolag='AIC')
        badge_r = "result-pass" if adf_ret[1] < 0.05 else "result-fail"
        label_r = "Stationary ✅" if adf_ret[1] < 0.05 else "Non-Stationary ❌"
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-value">{adf_ret[1]:.6f}</div>
            <div class="metric-label">ADF p-value — Stock Returns — <span class="{badge_r}">{label_r}</span></div>
        </div>
        """, unsafe_allow_html=True)

    st.markdown("""
    <div class="insight-box">
        <strong>💡 Key Observation:</strong> Stock <em>prices</em> are non-stationary (they trend up/down).
        But daily <em>returns</em> (percentage changes) are typically stationary — they fluctuate around zero.
        This means for returns, we likely need <strong>d=0</strong> (no differencing), which simplifies ARIMA to ARMA.
    </div>
    """, unsafe_allow_html=True)

    # --- ACF/PACF ---
    st.markdown('<div class="section-header"><h2>Step 2: ACF & PACF of Returns</h2></div>', unsafe_allow_html=True)

    n_lags_stock = st.slider("Number of lags:", 10, 40, 20, key="stock_lags")
    returns_clean = stock_returns['Return'].dropna().values

    acf_vals = acf(returns_clean, nlags=n_lags_stock)
    pacf_vals = pacf(returns_clean, nlags=n_lags_stock, method='ywm')
    ci_val = 1.96 / np.sqrt(len(returns_clean))

    col1, col2 = st.columns(2)
    with col1:
        fig_acf = go.Figure()
        for i in range(len(acf_vals)):
            color = '#e74c3c' if abs(acf_vals[i]) > ci_val and i > 0 else '#0e7c6b'
            fig_acf.add_trace(go.Bar(x=[i], y=[acf_vals[i]], marker_color=color, showlegend=False, width=0.4))
        fig_acf.add_hline(y=ci_val, line_dash='dash', line_color='#95a5a6')
        fig_acf.add_hline(y=-ci_val, line_dash='dash', line_color='#95a5a6')
        fig_acf.update_layout(title='ACF of Stock Returns → helps determine q', height=350,
                              template='plotly_white', font=dict(family='Inter'), title_font_size=13,
    ...
    with col2:
        ...
        for i in range(len(pacf_vals)):
            color = '#e74c3c' if abs(pacf_vals[i]) > ci_val and i > 0 else '#0e7c6b'
            fig_pacf.add_trace(go.Bar(x=[i], y=[pacf_vals[i]], marker_color=color, showlegend=False, width=0.4))
        fig_pacf.add_hline(y=ci_val, line_dash='dash', line_color='#95a5a6')
        fig_pacf.add_hline(y=-ci_val, line_dash='dash', line_color='#95a5a6')
        fig_pacf.update_layout(title='PACF of Stock Returns → helps determine p', height=350,
                              template='plotly_white', font=dict(family='Inter'), title_font_size=13,
    ...
    st.markdown("""
    <div class="insight-box">
        <strong>💡 Compare with the airline data:</strong> The airline ACF showed strong, slowly decaying autocorrelation
        with seasonal spikes at lag 12. Stock returns typically show very few (if any) significant lags — most bars
        stay within the confidence bands. This tells us there's very little predictable structure for ARIMA to learn.
    </div>
    """, unsafe_allow_html=True)

    # --- Train/Test Split ---
    st.markdown('<div class="section-header"><h2>Step 3: Train/Test Split</h2></div>', unsafe_allow_html=True)

    split_pct_ts = st.slider("Training data percentage:", 60, 90, 80, key="ts_split")
    split_idx_ts = int(len(stock_returns) * split_pct_ts / 100)
    train_ts = stock_returns[:split_idx_ts]
    test_ts = stock_returns[split_idx_ts:]

    fig_split_ts = go.Figure()
    fig_split_ts.add_trace(go.Scatter(x=train_ts['Date'], y=train_ts['Return'], mode='lines',
                                       name=f'Train ({len(train_ts)} days)', line=dict(color='#0e7c6b', width=1.5)))
    fig_split_ts.add_trace(go.Scatter(x=test_ts['Date'], y=test_ts['Return'], mode='lines',
                                       name=f'Test ({len(test_ts)} days)', line=dict(color='#e74c3c', width=1.5)))
    fig_split_ts.update_layout(title='Train/Test Split — Daily Returns', height=300, template='plotly_white',
                                font=dict(family='Inter'), title_font_size=14, hovermode='x unified')
    st.plotly_chart(fig_split_ts, use_container_width=True)

    # --- ARIMA ---
    st.markdown('<div class="section-header"><h2>Step 4: Fit ARIMA Model</h2></div>', unsafe_allow_html=True)

    st.markdown("**Select ARIMA(p, d, q) parameters:**")
    col1, col2, col3 = st.columns(3)
    with col1:
        ap = st.selectbox("p (AR order):", [0,1,2,3,4,5], index=1, key="arima_p")
    with col2:
        ad = st.selectbox("d (Differencing):", [0,1,2], index=0, key="arima_d")
    with col3:
        aq = st.selectbox("q (MA order):", [0,1,2,3,4,5], index=1, key="arima_q")

    if st.button("🔧 Fit ARIMA", use_container_width=True, type="primary"):
        with st.spinner(f"Fitting ARIMA({ap},{ad},{aq})..."):
            try:
                arima_m = ARIMA(train_ts['Return'].values, order=(ap, ad, aq)).fit()
                arima_fc = arima_m.forecast(steps=len(test_ts))

                arima_mae = mean_absolute_error(test_ts['Return'], arima_fc)
                arima_rmse = np.sqrt(mean_squared_error(test_ts['Return'], arima_fc))
                arima_r2 = r2_score(test_ts['Return'], arima_fc)
                arima_dir = (np.sign(test_ts['Return'].values) == np.sign(arima_fc)).mean() * 100

                st.session_state['arima_metrics'] = {'MAE': arima_mae, 'RMSE': arima_rmse, 'R²': arima_r2, 'Dir': arima_dir}
                st.session_state['arima_pred'] = arima_fc
                st.session_state['arima_test_dates'] = test_ts['Date'].values
                st.session_state['arima_y_test'] = test_ts['Return'].values
                st.session_state['arima_order_str'] = f"({ap},{ad},{aq})"
                st.session_state['arima_aic'] = arima_m.aic
                st.session_state['arima_bic'] = arima_m.bic

                st.success(f"ARIMA({ap},{ad},{aq}) fitted! AIC: {arima_m.aic:.1f} | BIC: {arima_m.bic:.1f}")

                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.markdown(f"""<div class="metric-card"><div class="metric-value">{arima_mae:.6f}</div><div class="metric-label">MAE</div></div>""", unsafe_allow_html=True)
                with col2:
                    st.markdown(f"""<div class="metric-card"><div class="metric-value">{arima_rmse:.6f}</div><div class="metric-label">RMSE</div></div>""", unsafe_allow_html=True)
                with col3:
                    r2c = '#166534' if arima_r2 > 0 else '#991b1b'
                    st.markdown(f"""<div class="metric-card"><div class="metric-value" style="color:{r2c}">{arima_r2:.4f}</div><div class="metric-label">R²</div></div>""", unsafe_allow_html=True)
                with col4:
                    st.markdown(f"""<div class="metric-card"><div class="metric-value">{arima_dir:.1f}%</div><div class="metric-label">Direction Acc.</div></div>""", unsafe_allow_html=True)

                # Forecast plot
                fig_arima = go.Figure()
                fig_arima.add_trace(go.Scatter(x=test_ts['Date'], y=test_ts['Return'], mode='lines', name='Actual', line=dict(color='#2c3e50', width=1.5)))
                fig_arima.add_trace(go.Scatter(x=test_ts['Date'], y=arima_fc, mode='lines', name=f'ARIMA{st.session_state["arima_order_str"]}', line=dict(color='#e74c3c', width=2, dash='dash')))
                fig_arima.update_layout(title=f'ARIMA{st.session_state["arima_order_str"]} Forecast vs Actual Returns',
                                        height=380, template='plotly_white', font=dict(family='Inter'), title_font_size=14, hovermode='x unified')
                st.plotly_chart(fig_arima, use_container_width=True)

                st.markdown("""
                <div class="warning-box">
                    <strong>⚠️ Notice the pattern:</strong> ARIMA's forecast on stock returns tends to converge quickly toward
                    a flat line (near zero). This is because returns lack persistent autocorrelation — once the AR/MA effects
                    die out after a few steps, the forecast reverts to the mean. This is expected behavior, not a bug.
                </div>
                """, unsafe_allow_html=True)

            except Exception as e:
                st.error(f"ARIMA fitting failed: {str(e)}. Try different p, d, q values.")

    # --- SARIMA ---
    st.markdown('<div class="section-header"><h2>Step 5: Fit SARIMA Model</h2></div>', unsafe_allow_html=True)

    st.markdown("""
    <div class="concept-card">
        <p>
        SARIMA adds seasonal components. For stock data, there's a hypothesis that returns might show
        <strong>weekly seasonality</strong> (s=5 for 5 trading days) — e.g., the "Monday effect" or "Friday effect."
        Let's test it.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        sp = c1.selectbox("p", [0,1,2,3], index=1, key="sarima_sp")
        sd = c2.selectbox("d", [0,1], index=0, key="sarima_sd")
        sq = c3.selectbox("q", [0,1,2,3], index=1, key="sarima_sq")
    with col2:
        st.markdown("**Seasonal (P, D, Q, s)**")
        c4, c5, c6, c7 = st.columns(4)
        sP = c4.selectbox("P", [0,1,2], index=1, key="sarima_sP")
        sD = c5.selectbox("D", [0,1], index=0, key="sarima_sD")
        sQ = c6.selectbox("Q", [0,1,2], index=1, key="sarima_sQ")
        ss = c7.selectbox("s", [5, 10, 21], index=0, key="sarima_ss",
                          help="5=weekly, 10=biweekly, 21=monthly trading days")

    if st.button("🔧 Fit SARIMA", use_container_width=True, type="primary"):
        with st.spinner(f"Fitting SARIMA({sp},{sd},{sq})({sP},{sD},{sQ},{ss}) — may take a moment..."):
            try:
                sarima_m = SARIMAX(
                    train_ts['Return'].values, order=(sp, sd, sq),
                    seasonal_order=(sP, sD, sQ, ss),
                    enforce_stationarity=False, enforce_invertibility=False
                ).fit(disp=False)

                sarima_fc = sarima_m.forecast(steps=len(test_ts))

                sarima_mae = mean_absolute_error(test_ts['Return'], sarima_fc)
                sarima_rmse = np.sqrt(mean_squared_error(test_ts['Return'], sarima_fc))
                sarima_r2 = r2_score(test_ts['Return'], sarima_fc)
                sarima_dir = (np.sign(test_ts['Return'].values) == np.sign(sarima_fc)).mean() * 100

                st.session_state['sarima_stock_metrics'] = {'MAE': sarima_mae, 'RMSE': sarima_rmse, 'R²': sarima_r2, 'Dir': sarima_dir}
                st.session_state['sarima_stock_pred'] = sarima_fc
                st.session_state['sarima_stock_test_dates'] = test_ts['Date'].values
                st.session_state['sarima_stock_y_test'] = test_ts['Return'].values
                st.session_state['sarima_stock_order_str'] = f"({sp},{sd},{sq})({sP},{sD},{sQ},{ss})"
                st.session_state['sarima_stock_aic'] = sarima_m.aic
                st.session_state['sarima_stock_bic'] = sarima_m.bic

                st.success(f"SARIMA fitted! AIC: {sarima_m.aic:.1f} | BIC: {sarima_m.bic:.1f}")

                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.markdown(f"""<div class="metric-card"><div class="metric-value">{sarima_mae:.6f}</div><div class="metric-label">MAE</div></div>""", unsafe_allow_html=True)
                with col2:
                    st.markdown(f"""<div class="metric-card"><div class="metric-value">{sarima_rmse:.6f}</div><div class="metric-label">RMSE</div></div>""", unsafe_allow_html=True)
                with col3:
                    r2c = '#166534' if sarima_r2 > 0 else '#991b1b'
                    st.markdown(f"""<div class="metric-card"><div class="metric-value" style="color:{r2c}">{sarima_r2:.4f}</div><div class="metric-label">R²</div></div>""", unsafe_allow_html=True)
                with col4:
                    st.markdown(f"""<div class="metric-card"><div class="metric-value">{sarima_dir:.1f}%</div><div class="metric-label">Direction Acc.</div></div>""", unsafe_allow_html=True)

                # Forecast plot with both ARIMA and SARIMA
                fig_sarima = go.Figure()
                fig_sarima.add_trace(go.Scatter(x=test_ts['Date'], y=test_ts['Return'], mode='lines', name='Actual', line=dict(color='#2c3e50', width=1.5)))
                if 'arima_pred' in st.session_state:
                    fig_sarima.add_trace(go.Scatter(x=test_ts['Date'], y=st.session_state['arima_pred'], mode='lines',
                                                     name=f'ARIMA{st.session_state["arima_order_str"]}', line=dict(color='#e74c3c', width=1.5, dash='dot')))
                fig_sarima.add_trace(go.Scatter(x=test_ts['Date'], y=sarima_fc, mode='lines',
                                                 name=f'SARIMA{st.session_state["sarima_stock_order_str"]}', line=dict(color='#8b5cf6', width=2, dash='dash')))
                fig_sarima.update_layout(title='ARIMA vs SARIMA Forecast on Stock Returns',
                                          height=400, template='plotly_white', font=dict(family='Inter'), title_font_size=14,
                                          hovermode='x unified', legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='center', x=0.5))
                st.plotly_chart(fig_sarima, use_container_width=True)

            except Exception as e:
                st.error(f"SARIMA fitting failed: {str(e)}. Try reducing P, Q, or changing s.")

    # --- ARIMA vs SARIMA comparison ---
    if 'arima_metrics' in st.session_state and 'sarima_stock_metrics' in st.session_state:
        st.markdown('<div class="section-header"><h2>ARIMA vs SARIMA on Stock Returns</h2></div>', unsafe_allow_html=True)

        am = st.session_state['arima_metrics']
        ssm = st.session_state['sarima_stock_metrics']

        comp_ts = pd.DataFrame({
            'Metric': ['MAE', 'RMSE', 'R²', 'Direction Accuracy'],
            f'ARIMA{st.session_state["arima_order_str"]}': [f"{am['MAE']:.6f}", f"{am['RMSE']:.6f}", f"{am['R²']:.4f}", f"{am['Dir']:.1f}%"],
            f'SARIMA{st.session_state["sarima_stock_order_str"]}': [f"{ssm['MAE']:.6f}", f"{ssm['RMSE']:.6f}", f"{ssm['R²']:.4f}", f"{ssm['Dir']:.1f}%"],
        }).set_index('Metric')
        st.dataframe(comp_ts, use_container_width=True)

    # --- Key Insight ---
    st.markdown('<div class="section-header"><h2>The Takeaway</h2></div>', unsafe_allow_html=True)

    st.markdown("""
    <div class="concept-card">
        <h4>Why ARIMA/SARIMA struggle with stock returns</h4>
        <p>
        <strong>1. Near-random-walk behavior:</strong> Stock returns have very weak autocorrelation. ARIMA relies on
        autocorrelation to make predictions — with little structure to learn, it forecasts near the mean (≈0).<br><br>
        <strong>2. No strong seasonality:</strong> Unlike airline passengers with clear 12-month cycles, stock returns
        don't have reliable periodic patterns. SARIMA's seasonal component has little to latch onto.<br><br>
        <strong>3. Exogenous drivers matter:</strong> Stock prices are moved by news, earnings, macroeconomics, and
        investor behavior — factors that are <em>external</em> to the price series. ARIMA only sees the series itself.<br><br>
        <strong>4. This is exactly why we added sentiment:</strong> Regression models with sentiment features can
        incorporate external information that ARIMA/SARIMA cannot. The comparison on the next page makes this concrete.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class="insight-box">
        <strong>💡 The lesson:</strong> The right model depends on the data. SARIMA excels when data has trend + seasonality
        (airline passengers). For stock returns, models that incorporate external features (news, sentiment, fundamentals)
        often outperform pure time series approaches. Go to <strong>⚔️ Head-to-Head</strong> to see the full 4-model comparison.
    </div>
    """, unsafe_allow_html=True)


# ═══════════════════════════════════════════════════════════════
# PAGE: HEAD-TO-HEAD — ALL MODELS
# ═══════════════════════════════════════════════════════════════
elif page == "⚔️ Head-to-Head: All Models":
    st.markdown("""
    <div class="main-header">
        <h1>⚔️ Head-to-Head: All Four Approaches</h1>
        <p>ARIMA vs SARIMA vs Regression (Price-Only) vs Regression (Price+Sentiment) — which works best for stock returns?</p>
    </div>
    """, unsafe_allow_html=True)

    # Check what's available
    has_price = 'price_metrics' in st.session_state
    has_sent = 'sent_metrics' in st.session_state
    has_arima = 'arima_metrics' in st.session_state
    has_sarima = 'sarima_stock_metrics' in st.session_state

    trained_count = sum([has_price, has_sent, has_arima, has_sarima])

    if trained_count < 2:
        st.markdown("""
        <div class="warning-box">
            <strong>⚠️ Train at least 2 models to compare!</strong><br><br>
            Go to the following sections and train models:<br>
            • <strong>💹 Stock Prediction — Without Sentiment</strong> (Regression: price-only)<br>
            • <strong>🧠 Stock Prediction — With Sentiment</strong> (Regression: price + sentiment)<br>
            • <strong>📉 ARIMA/SARIMA on Stock Data</strong> (ARIMA and SARIMA)
        </div>
        """, unsafe_allow_html=True)
    else:
        # --- Build comparison data ---
        st.markdown('<div class="section-header"><h2>Accuracy Metrics — All Models</h2></div>', unsafe_allow_html=True)

        models = {}
        colors = {}
        if has_arima:
            am = st.session_state['arima_metrics']
            label = f"ARIMA{st.session_state['arima_order_str']}"
            models[label] = am
            colors[label] = '#e74c3c'
        if has_sarima:
            ssm = st.session_state['sarima_stock_metrics']
            label = f"SARIMA{st.session_state['sarima_stock_order_str']}"
            models[label] = ssm
            colors[label] = '#8b5cf6'
        if has_price:
            pm = st.session_state['price_metrics']
            pm['Dir'] = (np.sign(st.session_state['price_y_test']) == np.sign(st.session_state['price_pred'])).mean() * 100
            label = f"Regression ({st.session_state['price_model_name']})"
            models[label] = pm
            colors[label] = '#f59e0b'
        if has_sent:
            sm = st.session_state['sent_metrics']
            sm['Dir'] = (np.sign(st.session_state['sent_y_test']) == np.sign(st.session_state['sent_pred'])).mean() * 100
            label = f"Reg+Sentiment ({st.session_state['sent_model_name']})"
            models[label] = sm
            colors[label] = '#0e7c6b'

        # Metrics table
        comp_data = {'Metric': ['MAE', 'RMSE', 'R²', 'Direction Accuracy (%)']}
        for name, m in models.items():
            comp_data[name] = [
                f"{m['MAE']:.6f}",
                f"{m['RMSE']:.6f}",
                f"{m['R²']:.4f}",
                f"{m.get('Dir', 'N/A'):.1f}" if isinstance(m.get('Dir'), (int, float)) else 'N/A'
            ]

        comp_df = pd.DataFrame(comp_data).set_index('Metric')

        # Find winners
        mae_vals = {n: m['MAE'] for n, m in models.items()}
        rmse_vals = {n: m['RMSE'] for n, m in models.items()}
        r2_vals = {n: m['R²'] for n, m in models.items()}
        dir_vals = {n: m.get('Dir', 0) for n, m in models.items()}
        ...
        <div class="insight-box">
            ...
            Best MAE: <strong>{best_mae}</strong> ({mae_vals[best_mae]:.6f})<br>
            Best RMSE: <strong>{best_rmse}</strong> ({rmse_vals[best_rmse]:.6f})<br>
            Best R²: <strong>{best_r2}</strong> ({r2_vals[best_r2]:.4f})<br>
            Best Direction: <strong>{best_dir}</strong> ({dir_vals[best_dir]:.1f}%)
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            fig_mae = go.Figure(go.Bar(
                x=model_names, y=[models[n]['MAE'] for n in model_names],
                marker_color=model_colors,
                text=[f"{models[n]['MAE']:.6f}" for n in model_names],
                textposition='outside'
            ))
            fig_mae.update_layout(title='MAE (lower = better)', height=380, template='plotly_white',
                                   font=dict(family='Inter'), title_font_size=14, xaxis_tickangle=-20)
            st.plotly_chart(fig_mae, use_container_width=True)

        with col2:
            fig_r2 = go.Figure(go.Bar(
                x=model_names, y=[models[n]['R²'] for n in model_names],
                marker_color=model_colors,
                text=[f"{models[n]['R²']:.4f}" for n in model_names],
                textposition='outside'
            ))
            fig_r2.update_layout(title='R² Score (higher = better)', height=380, template='plotly_white',
                                  font=dict(family='Inter'), title_font_size=14, xaxis_tickangle=-20)
        ...
        fig_dir = go.Figure(go.Bar(
            x=model_names,
            y=[models[n].get('Dir', 0) for n in model_names],
            marker_color=model_colors,
            text=[f"{models[n].get('Dir', 0):.1f}%" for n in model_names],
            textposition='outside'
        ))
        fig_dir.add_hline(y=50, line_dash='dash', line_color='#94a3b8', annotation_text='Random guess (50%)')
        dir_max = max([models[n].get('Dir', 50) for n in model_names])
        fig_dir.update_layout(title='Direction Accuracy — Did the model predict Up/Down correctly? (higher = better)',
                              height=400, template='plotly_white', font=dict(family='Inter'), title_font_size=14,
                              xaxis_tickangle=-20, yaxis_range=[40, dir_max + 8])
        st.plotly_chart(fig_dir, use_container_width=True)

        # --- Prediction Overlay ---
        st.markdown('<div class="section-header"><h2>All Predictions Overlaid</h2></div>', unsafe_allow_html=True)

        fig_all = go.Figure()

        # Use the longest available test set as the actual line
        if has_price:
            fig_all.add_trace(go.Scatter(x=st.session_state['price_test_dates'], y=st.session_state['price_y_test'],
                                          mode='lines', name='Actual', line=dict(color='#2c3e50', width=2)))
        elif has_arima:
            fig_all.add_trace(go.Scatter(x=st.session_state['arima_test_dates'], y=st.session_state['arima_y_test'],
                                          mode='lines', name='Actual', line=dict(color='#2c3e50', width=2)))

        if has_arima:
            fig_all.add_trace(go.Scatter(x=st.session_state['arima_test_dates'], y=st.session_state['arima_pred'],
                                          mode='lines', name=f'ARIMA{st.session_state["arima_order_str"]}',
                                          line=dict(color='#e74c3c', width=1.5, dash='dot')))
        if has_sarima:
            fig_all.add_trace(go.Scatter(x=st.session_state['sarima_stock_test_dates'], y=st.session_state['sarima_stock_pred'],
                                          mode='lines', name=f'SARIMA{st.session_state["sarima_stock_order_str"]}',
                                          line=dict(color='#8b5cf6', width=1.5, dash='dashdot')))
        if has_price:
            fig_all.add_trace(go.Scatter(x=st.session_state['price_test_dates'], y=st.session_state['price_pred'],
                                          mode='lines', name='Regression (Price)',
                                          line=dict(color='#f59e0b', width=1.5, dash='dash')))
        if has_sent:
            fig_all.add_trace(go.Scatter(x=st.session_state['sent_test_dates'], y=st.session_state['sent_pred'],
                                          mode='lines', name='Reg+Sentiment',
                                          line=dict(color='#0e7c6b', width=1.5, dash='dash')))

        fig_all.update_layout(
            title='All Models vs Actual Returns', height=500, template='plotly_white',
            font=dict(family='Inter'), title_font_size=15, hovermode='x unified', yaxis_title='Daily Return',
            legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='center', x=0.5)
        )
        st.plotly_chart(fig_all, use_container_width=True)

        # --- Overall Verdict ---
        st.markdown('<div class="section-header"><h2>The Big Picture</h2></div>', unsafe_allow_html=True)

        # Count wins per model
        winners = [best_mae, best_rmse, best_r2, best_dir]
        from collections import Counter
        win_counts = Counter(winners)
        overall_winner = win_counts.most_common(1)[0][0]
        overall_wins = win_counts.most_common(1)[0][0]

        # Classify the winner type
        is_ts_model = any(x in overall_winner.lower() for x in ['arima', 'sarima'])
        is_sentiment = 'sentiment' in overall_winner.lower()
        ...
        <div style="background:linear-gradient(135deg,#ecfdf5,#d1fae5); border:2px solid #6ee7b7; border-radius:12px; padding:1.5rem; margin:1rem 0;">
            <strong style="color:#065f46; font-size:1.2rem;">🏆 Overall Winner: {overall_winner}</strong> ({win_counts[overall_winner]}/4 metrics)<br><br>
            <strong>Win distribution:</strong> {', '.join([f'{name}: {count}/4' for name, count in win_counts.most_common()])}
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="concept-card">
            ...
            <p>
            <strong>ARIMA/SARIMA on stock returns:</strong> These models are designed for data with strong autocorrelation
            and/or seasonality. Stock returns have neither — so they tend to forecast near zero (the mean), which is
            safe but uninformative. They work beautifully on airline passenger data but are the wrong tool here.<br><br>

            <strong>Regression with price features:</strong> By engineering features like momentum, volatility, and
            moving average ratios, we give the model explicit signals about market conditions. This is more informative
            than raw autocorrelation.<br><br>

            <strong>Regression with sentiment:</strong> Adding news sentiment provides external, qualitative information
            that no price-based model can capture. Even modest improvements in direction accuracy can matter in practice.<br><br>

            <strong>The lesson:</strong> There is no universally "best" model. The right choice depends on the
            <em>characteristics of your data</em>. Time series structure → ARIMA/SARIMA. External drivers + engineered features
            → Regression/ML. The best practitioners use multiple approaches and understand why each works (or doesn't).
            </p>
        </div>
        """, unsafe_allow_html=True)
